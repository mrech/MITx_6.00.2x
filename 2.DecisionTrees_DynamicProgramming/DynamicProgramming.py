...
def fastMaxVal(toConsider, avail, memo={}):
    ...
    # if the items are in the memo
    if (len(toConsider), avail) in memo:
        # we return it
        result = memo[(len(toConsider), avail)]
    # if there is nothing left to consider
    elif toConsider == [] or avail == 0:
        # return an empy tuple
        result = (0, ())
    elif toConsider[0].getCost() > avail:
        # Explore right branch only
        ...
        if withVal > withoutVal:
            ...
            result = (withoutVal, withoutToTake)
    # Before we return we update the memo
...
def testMaxVal(foods, maxUnits, algorithm, printItems=True):
    ...
    print('Use search tree to allocate', maxUnits,
          'calories')
    # allow us to use the same testing function to test different
...
for numItems in (5, 10, 15, 20, 25, 30, 35, 40, 45, 50):
   ...
   testMaxVal(items, 750, fastMaxVal, False) # test with maxVal and fastMaxVal

'''
For numItems > 1000
RecursionError: maximum recursion depth exceeded in comparison.
Python saves information about the function making the call on the
call stack. The runtime system places a limit on the depth of recursion.
